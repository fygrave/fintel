...
# source https://github.com/ChrisDelClea/Inspirationals/blob/main/main.py
...
def app():
  ...
  query_type = st.sidebar.selectbox("Intel Query: ", ["Searches", "Cases", "Huieises"]) # could add more stuff here later on or add other endpoints in the sidebar.
  config = Config(height=600, width=700, nodeHighlightBehavior=True, highlightColor="#F7A7A6", directed=True,
                  collapsible=True)

  if query_type=="Searches":
